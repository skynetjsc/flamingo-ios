                        ...
                        DispatchQueue.main.async(execute: {
                            ...
                            if (result[0]["BookingID"] != nil) {
                                ...
                                alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { (UIAlertAction) in
                                    guard let navigationController = self.sideMenuController?.contentViewController as? UINavigationController else {
                                        return
                                    }
                                    guard let viewController = self.storyboard?.instantiateViewController(withIdentifier: "TrackBookPaymentController") as? NavigationController else {
                                        return
                                    }
                                    navigationController.present(viewController, animated: true, completion: nil)
                                }))
